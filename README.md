...
v0.2.2는 Labels 시작 시 설치된 원본 Codex 버전을 한 번 읽습니다. Labels를 만든 기반 버전과 다르면 업데이트 영역에 **새 Codex 버전 감지됨 / Labels 업데이트 확인**을 표시합니다. 호환성은 미확인으로 안내하며 이 감지만으로 설치·패치·재시작하지 않습니다. 버튼을 눌러야 기존 Labels 릴리스를 조회합니다. 원본이 PC에 설치된 뒤 감지하며, Store의 미설치 업데이트를 조회하는 기능은 아닙니다.
