...
Windows x64 Microsoft Store Codex는 **버전에 관계없이** 사용합니다. 고정 버전 대신 설치된 앱에 Labels가 연결할 구조가 있는지 확인하며, 마지막으로 실제 실행을 확인한 버전은 내부 앱 `26.917.51856`입니다. v0.3.4부터 배포 ZIP도 버전에 관계없이 동작합니다(v0.3.3 이하 ZIP은 26.915 전용). 소스에서 준비하려면 Python 3.11 이상이 필요하며 Node.js 22 이상은 테스트용입니다. 연결할 구조가 없는 앱과 이미 패치된 설치본은 거부합니다. 백그라운드 작업 활동 연결부를 찾지 못한 버전은 그 기능만 빼고 라벨·단어장·계정 창을 준비합니다. 설치본의 Owl 런타임을 그대로 사용하며 일반 Electron 실행 파일로 교체하지 않습니다.
