...
v0.2.0부터 **라벨 설정… → Codex Labels 업데이트 → 업데이트 확인**에서 새 정식 릴리스를 확인합니다. **다운로드 및 다음 실행에 적용**을 누른 뒤, Labels를 완전히 종료하고 바탕화면 바로가기로 다시 실행하면 적용됩니다. 자동 종료·백그라운드 다운로드는 하지 않습니다. 개인 설정·할당을 유지하고 이전 실행본을 백업합니다. v0.1.0 사용자는 새 ZIP을 같은 폴더에 풀고 **설치.cmd**를 실행해 한 번 업그레이드해야 합니다. 설정 자동 동기화는 제공하지 않습니다. [다른 PC 설치와 업데이트 안내](docs/windows-install.md)를 참고하세요.
