...
- 기존 Codex 홈(`CODEX_HOME`, 미지정 시 사용자 홈의 `.codex`)을 사용합니다. 파일 기반 로그인 정보가 없으면 로그인 오류를 표시합니다. 계정별 창 애드온 자체는 별도 기능이며 이 브랜치에 포함하지 않습니다. 호스트가 로그인 홈과 저장 폴더를 분리해 제공하면 같은 단어장 모듈을 사용할 수 있습니다.
...
- `vocabulary.json`을 Labels 설치 폴더의 라벨 설정과 나란히 보관합니다. 저장 폴더별로 단어장이 분리되며, 대화별 저장은 아닙니다.
