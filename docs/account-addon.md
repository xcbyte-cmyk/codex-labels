...
단어장 v2를 기본 모듈로 통합했다. a 전용 활성화 분기와 중복 v2 모듈은 제거했다.
모든 계정에서 같은 기능을 제공하지만 각 계정의 directory/home을 유지하므로 데이터와 로그인은 공유하지 않는다.
실행 앱은 소스 병합만으로 갱신되지 않는다. 기존 앱과 a 시험본은 별도 적용 전까지 현재 실행 버전을 유지한다.
처음 v2 쓰기 시 기존 v1 자료가 백업된다. 되돌리기는 코드와 자료 버전을 함께 검토해야 한다.
