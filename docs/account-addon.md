...
사용자가 제공한 codex-labels-global-vocabulary.zip의 단어장 v2를 별도 모듈로 포함한다.
계정 account.json의 vocabularyVersion이 정수 2일 때만 main이 v2 IPC와 renderer를 선택한다.
기본 창 및 해당 값이 없는 다른 계정은 기존 단어장 모듈을 계속 사용한다.
전역은 같은 계정 저장소 내 여러 대화를 뜻하며 다른 계정과 단어장을 공유하지 않는다.
처음 v2 쓰기 시 기존 v1 단어장은 제공 구현에 따라 같은 폴더에 백업된다.
모듈을 구버전으로 되돌리기 전에 v2 쓰기 여부와 데이터 복구를 함께 확인해야 한다.
계정 선택 필드는 renderer 입력에서 받지 않는다.
