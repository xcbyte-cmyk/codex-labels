...
v0.2.2부터 시작 시 원본 설치 버전을 로컬에서 한 번 확인한다. Labels 기반 버전과 다르면 설정창 업데이트 영역이 펼쳐지고 버전 차이와 호환성 미확인 안내를 표시한다. 이 감지는 현재 실행본을 바꾸지 않는다. Labels 업데이트 확인 버튼은 기존 공개 릴리스 조회만 수행하며 자동 다운로드·자동 패치·자동 재시작은 하지 않는다. 조회 실패는 앱 실행을 막지 않고, 다음 Labels 실행 시 다시 감지한다. 이미 설치된 원본만 비교하므로 Microsoft Store에만 올라온 새 버전은 감지 대상이 아니다.
...
업데이트는 고정된 xcbyte-cmyk/codex-labels의 GitHub 정식 릴리스만 조회한다. GitHub API의 SHA-256 digest와 다운로드 길이를 확인하고 ZIP 파일 허용 목록, 패키지 버전과 실제 설치된 Codex 버전의 호환성을 검사한다. 개인 설정·할당·프로필과 공식 Codex는 업데이트 파일 목록에 포함되지 않는다. 네트워크/호환성/검증 실패는 현재 설치를 바꾸지 않는다. 실행 도구 교체 실패 시 이전 파일을 복원한다. `.updates/stage-*/previous`에 직전 실행 도구와 메타데이터를 보관한다.
