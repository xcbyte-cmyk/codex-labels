...
라벨 설정 → Codex Labels 업데이트 → 업데이트 확인을 누른다. 다운로드 완료 뒤 **설치하고 다시 실행**을 누르면 별도 진행 창이 현재 Labels의 정상 종료를 기다려 설치하고 앱을 다시 연다. 열린 작업이 중단될 수 있다는 안내를 보여주며, 저장하지 않은 라벨 편집은 먼저 저장·취소해야 한다. 강제 프로세스 종료는 하지 않는다. 설치 도구가 준비되지 않으면 기존 앱을 종료하지 않는다. 설정창을 열 때 로컬 설치 상태만 읽고, 확인 버튼을 누르기 전에는 네트워크 요청이나 다운로드를 하지 않는다. 같은 버전·이전 버전·시험판은 자동 설치 대상이 아니다.
