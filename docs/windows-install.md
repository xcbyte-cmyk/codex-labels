...
v0.2.0부터 라벨 설정 → Codex Labels 업데이트 → 업데이트 확인을 누른다. 새 버전이 있으면 다운로드 및 다음 실행에 적용을 누르고 Labels를 완전히 종료한 뒤 바로가기로 다시 연다. 다른 작업을 중단시키지 않도록 앱을 강제 종료하지 않는다. 확인 버튼을 누르기 전에는 네트워크 요청이나 다운로드를 하지 않는다. 같은 버전·이전 버전·시험판은 자동 설치 대상이 아니다.
...
다음 실행에서는 이전 runtime/app을 runtime/app.backup-날짜-식별자로 남기고 새 사본을 준비한다. 새 사본 준비 실패 시 이전 runtime과 빌드 기록을 복원하며 오류를 안내한다. Labels를 아직 닫지 않았다면 기존 앱을 종료하라는 안내 후 중단한다. 이전 실행 도구가 필요하면 Labels를 닫고 위 previous 폴더의 두 파일을 설치 루트로 복사해 복구할 수 있다.
