...
v0.2.3부터 화면의 알림 오류 메시지에 닫기 버튼과 Esc 닫기를 제공하며, 12초 후 자동으로 숨긴다. 같은 오류가 다시 발생하면 표시 시간을 갱신하고, 재시도가 성공하면 이전 오류를 제거한다. 화면을 닫을 때 타이머와 키보드 리스너도 해제한다. 메시지를 닫는 동작은 Windows 알림 등록 자체를 변경하지 않는다.
