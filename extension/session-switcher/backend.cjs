...
function rolloutRelative(file,home,id) {
  const relative=path.relative(fs.realpathSync(home),fs.realpathSync(file));
  const parts=relative.split(path.sep);
  if(parts.length!==5 || parts[0]!=='sessions' || !/^\d{4}$/.test(parts[1]) ||
    !/^\d{2}$/.test(parts[2]) || !/^\d{2}$/.test(parts[3]) ||
    !parts[4].startsWith('rollout-') || !parts[4].endsWith(`-${id}.jsonl`)) fail('CHECKPOINT_INVALID');
  return relative;
}
...
class DesktopBackend {
  ...
  async getActivity() {
    ...
    const entries=Array.isArray(terminals?.terminals)?terminals.terminals:
      Array.isArray(terminals?.data)?terminals.data:null;
    if(!entries || terminals?.nextCursor) fail('UNSUPPORTED');
    c.runningCommands+=entries.length;
  ...
  async checkpoint({signal}={}) {
    ...
    const relativeRollout=rolloutRelative(record.thread.path,this.home,this.logicalSessionId);
    checkAbort(signal);if(!idle(await this.getActivity())) fail('BUSY');
    ...
    return {version:1,logicalSessionId:this.logicalSessionId,payload:{log,sha256:digest(log),settings,relativeRollout}};
  }
  async restore(checkpoint,{signal}={}) {
    ...
    const {log,sha256,settings,relativeRollout}=checkpoint.payload||{};
    if(digest(log||'')!==sha256) fail('CHECKPOINT_INVALID');
    validateRollout(log,this.logicalSessionId);
    const pieces=typeof relativeRollout==='string'?relativeRollout.split(path.sep):[];
    if(pieces.length!==5 || pieces[0]!=='sessions' || !/^\d{4}$/.test(pieces[1]) ||
      !/^\d{2}$/.test(pieces[2]) || !/^\d{2}$/.test(pieces[3]) ||
      !pieces[4].startsWith('rollout-') || !pieces[4].endsWith(`-${this.logicalSessionId}.jsonl`)) fail('CHECKPOINT_INVALID');
    const file=path.join(this.home,...pieces);
    fs.mkdirSync(path.dirname(file),{recursive:true,mode:0o700});
    fs.writeFileSync(file,log,{flag:'wx',mode:0o600});
    const result=await this.rpc.call('thread/resume',{...settings.params,threadId:this.logicalSessionId});
  ...
  async resumeSaved(saved) {
    ...
    rolloutRelative(file,this.home,this.logicalSessionId);
    const result=await this.rpc.call('thread/resume',{...saved.settings.params,threadId:this.logicalSessionId});
