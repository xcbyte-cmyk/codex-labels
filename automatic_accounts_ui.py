...
MESSAGES = {
    'checking': '대상 계정의 저장 로그인과 온라인 인증을 확인하고 있습니다.',
    'closing': 'Labels를 정상 종료하고 있습니다. 종료 확인창이 뜨면 확인해 주세요.',
    'activating': '종료를 확인했습니다. 선택한 로그인 정보를 적용하고 있습니다.',
    'verifying': '같은 작업 공간에서 선택 계정의 인증을 다시 확인하고 있습니다.',
    'reopening': '기존 작업 공간으로 Labels를 다시 실행하고 있습니다.',
    'done': '선택 계정의 인증 확인 후 Labels를 다시 실행했습니다. 실제 계정은 앱 메뉴에서도 확인하세요.',
    'FILE_STORE_REQUIRED': '파일 기반 로그인만 지원합니다. 현재 설정이 file인지 확인되지 않아 계정은 바꾸지 않았습니다. keyring/auto는 자동 변환하지 않습니다.',
    'ENV_AUTH_UNSUPPORTED': '다른 인증·API 라우팅 환경변수가 있습니다. 구독 계정 전환과 혼용하지 않습니다.',
    'OTHER_CODEX_RUNNING': '다른 Codex/CLI 또는 종료되지 않은 자식 프로세스가 있습니다. 해당 작업을 종료한 뒤 다시 시도하세요. 강제 종료하지 않습니다.',
...
class Picker:
    def __init__(self, ui, home, vault, verifier, desktop, *, recovery=False):
        ...
        ttk.Label(outer, text='계정 선택 → 정상 종료 → 인증 확인 → 같은 작업 공간 재실행\n처음 계정 추가 또는 로그인 만료 때만 브라우저 인증이 필요합니다.', wraplength=590).pack(anchor='w', pady=(8, 14))
        ...
        self.saved = tk.BooleanVar(value=False); self.consent = tk.BooleanVar(value=False)
        ttk.Checkbutton(outer, text='진행 중인 응답·도구 작업을 끝내고 필요한 초안을 저장했습니다.', variable=self.saved).pack(anchor='w', pady=(8, 2))
        ttk.Checkbutton(outer, text='기존 대화·코드 문맥이 선택한 계정으로 전송될 수 있음에 동의합니다.', variable=self.consent).pack(anchor='w')
    ...
    def switch(self):
        key = self.selected()
        if not key: return
        if not self.saved.get() or not self.consent.get(): self.status.set(MESSAGES['CONSENT_REQUIRED']); return
        name, _ = self.vault.get(key)
        if not messagebox.askyesno('계정 전환', f'[{name}] 계정으로 전환하고 Labels를 다시 실행할까요?', parent=self.ui): return
        handoff = Handoff(self.home, self.vault, self.verifier, self.desktop,
                          progress=lambda code: self.events.put(('progress', code)))
        self.start(lambda: handoff.switch(key, consent=True, work_saved=True), switching=True)
    def restore(self):
        if not messagebox.askyesno('이전 로그인 복원', '모든 Codex를 닫은 상태에서 이전 로그인으로 복원하고 이 후보를 다시 실행합니다.', parent=self.ui): return
